...
class XMLArmLoader(DL.DocumentLoader):
    def __init__(self, model_package="arm_1_0", ns_uri="http://www.cdisc.org/ns/arm/v1.0", local_model=False):
        self.filename = None
        self.parser = None
        if local_model:
    ...
    def load_document(self, elem, *args):
        ...
        if elem.text and not elem.text.isspace():
            ...
            odm_obj = eval("self.ARM." + elem_name + "(**" + str(attrib) + ")")
        else:
            odm_obj = eval("self.ARM." + elem_name + "(**" + str(elem.attrib) + ")")
        odm_obj_dict = eval("self.ARM." + elem_name + ".__dict__.items()")
